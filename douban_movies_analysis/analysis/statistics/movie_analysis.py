...
from utils.utils import get_movie_comments_dict, get_genre_dict, get_movie_info_dict_list, \
    get_movie_dataframe, get_movie_comments_words_dataframe, init_stopwords, plot2y
...
def comments_num_movies(rank_num=10):
    comments_num_statistics = dataframe.sort_values('comment_num', ascending=False)[
        ['movie_title', 'comment_num']].head(rank_num)
    return comments_num_statistics


# 分析投票数前rank_num名电影
def rating_people_num_movies(rank_num=10):
    rating_people_num_statistics = dataframe.sort_values('rating_people_num', ascending=False)[
        ['movie_title', 'rating_people_num']].head(rank_num)
    return rating_people_num_statistics


def plot_movie_years():
    plot2y(x_data=dataframe.year
           , x_label='release_year'
           , type1='hist'
           , y1_data=dataframe.comment_num
           , y1_color='#539caf'
           , y1_label='comment_num'
           , type2='bar'
           , y2_data=dataframe.rating_people_num
           , y2_color='#7663b0'
           , y2_label='rating_people_num'
           , title='The number of movie and their rating_num over Time')
    plt.savefig('movie_year_analysis.jpg')
    plt.show()
